...
async function run() {
    try {
        ...
        const serviceCollection = client.db('serviceCar').collection('service')
            ;
        const orderCollection = client.db('serviceCar').collection('order');
...
app.get('/', (req, res) => {
    res.send('running server car service')
})
